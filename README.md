...
> `scripts/update.py` 會以溫和公開抓取更新真實行情（Yahoo Auctions JP 成交作參考；香港最新賣出價是標題核對後的最低叫價，來自 Carousell、HKCardLink、LONO、ShipMyToy、Zenox）。求購／收卡價不算。各源筆數見 `meta.source_status`。`update_stub.py` 仍可產生範例資料。
